...
## util
##############
PS_PATH= os.getenv('PS_PATH', '../../../../') #path to PartialSpoof
...
ASVSPOOF_PROTOCAL= os.path.join(PS_PATH, 'project-NN-Pytorch-scripts.202102/project/02-asvspoof/DATA/asvspoof2019_LA/protocol.txt') #protocal of asvspoof2019
...
ssl_ckpt= os.path.join(PS_PATH, 'modules/ssl_pretrain/w2v_large_lv_fsh_swbd_cv.pt')
...
class Model(torch_nn.Module):
    ...
    def _weighted_sum(self, feature):
       ...
       # )
        min_len = min([f.shape[1] for f in feature])
        feature = [f[:, :min_len, :] for f in feature]
        stacked_feature = torch.stack(feature, dim=0)
        # edited by lds217
        # ensure all features have the same length
        # this is necessary because the weighted sum is calculated across all layers
        # and if the features have different lengths, the weighted sum will be incorrect
        # this is a workaround to ensure the weighted sum is correct
        # in the future, we should use a more correct approach to handle this
        # for example, we can use a padding approach to ensure all features have the same length
        # or we can use a more correct approach to handle this
...
#                    self.m_before_pooling)):
    ...
    def forward(self, x, fileinfo, dev_flag=False):
        ...
        if self.training:
            ...
            # by lds217 - print shape of each element in outs
            with open('outs_shape.txt', 'w') as f:
                f.write(f"Number of elements in outs: {len(outs)}\n")
                for i, o in enumerate(outs):
                    f.write(f"outs[{i}] shape: {o.shape}\n")
    ...
    def finish_up_inference(self):
        for i, fs_i in enumerate(Frame_shifts):
            ...
            #lds217 make a skip for 20ms, 40ms scale
            # 2 * 10ms = 20ms. Skip this scale.
            if fs_i == 2: 
                continue
            
            if fs_i == 4:
                # 4 * 10ms = 40ms. Skip this scale.
                continue
